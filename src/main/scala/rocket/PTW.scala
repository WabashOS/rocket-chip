...
class PTW(n: Int)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  val (s_ready :: s_req :: s_wait1 :: s_dummy1 :: s_wait2 :: s_wait3 :: s_dummy2 ::
       s_fragment_superpage :: s_pfareq :: s_pfawait :: Nil) = Enum(UInt(), 10)
  ...
  r_pte := OptimizationBarrier(MuxCase(r_pte, Seq(
    io.mem.resp.valid -> pte,
    l2_hit -> l2_pte,
    (state === s_fragment_superpage && !homogeneous) -> makePTE(fragmented_superpage_ppn, r_pte),
    (state === s_req && pte_cache_hit) -> makePTE(pte_cache_data, l2_pte),
    arb.io.out.fire() -> makePTE(io.dpath.ptbr.ppn, r_pte),
    io.pfa.resp.fire() -> new PTE().fromBits(io.pfa.resp.bits))))
